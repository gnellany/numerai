...
results = []
mainstart = time.time() #Create a start time variable
NAPI = numerapi.NumerAPI(verbosity="info") #Used to check current round number useless for training but great check for automation
DIR = "my_data_directory" #files location
full_path = f'{DIR}/numerai_dataset_{NAPI.get_current_round()}/' # get the full path with updated round
TARGET_NAME = f"target"
PREDICTION_NAME = f"prediction"
#MODEL_FILE = Path("Last_Fast_model.xgb") #Activate saving model & Change model name here

learning_rate = [0.003,0.005,0.007]
subsample = [0.3,2.5]
col_sample = [0.015,0.02]
max_depth = [7]
lamb = [0.0009]
apl = [23]
est = [10000]
...
def model(L_R,subsample,col,depth,lamb,apl,est):
    start = time.time()
    #print("Loading data...")
    # The training data is used to train your model how to predict the targets.
    training_data = read_csv(full_path +"numerai_training_data.csv")
    # The tournament data is the data that Numerai uses to evaluate your model.
    tournament_data = read_csv(full_path +"numerai_tournament_data.csv")
    ...
    model = XGBRegressor(n_estimators=est,learning_rate=L_R,subsample=subsample,colsample_bytree=col,max_depth=depth,
                         booster='gbtree',tree_method='gpu_hist',predictor='gpu_predictor',
                         reg_lambda=lamb,reg_alpha=apl,random_state=42)


    print("Training model...")
    model.fit(training_data[feature_names], training_data[TARGET_NAME])
    #model.save_model(MODEL_FILE) #Save a model
    ts2 = time.time() - start
    print(ts2)
    ...
    example_preds = pd.read_csv(full_path+"example_predictions.csv").set_index("id")["prediction"]
    ...
    #####
    results = [est,L_R,subsample,col,depth,lamb,apl,val_mmc_mean,corr_plus_mmc_sharpe,corr_plus_mmc_sharpe_diff,corr_with_example_preds]
    print(results)
    #append data here to save model data
    #####
    return results
...
def main():
    for o,n,m,l,k,j,i in itertools.product(range(len(est)),range(len(apl)),range(len(lamb)),range(len(max_depth)),
                                         range(len(col_sample)),range(len(subsample)),range(len(learning_rate))):
        if __name__ == '__main__':
            results.append(model(learning_rate[i],subsample[j],col_sample[k],max_depth[l],lamb[m],apl[n],est[o]))
            print(n,m,l,k,j,i)


f = open("model_results.csv", "a", newline="\n")
if __name__ == '__main__':
    main()

finish = time.time() -mainstart
writer = csv.writer(f)
