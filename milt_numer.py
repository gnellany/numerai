...
MODEL_FILE = Path("Last_Fast_model.xgb") #Change name here
...
def main(L_R,subsample,col,depth,lamb,apl):
    start = time.time()
    #print("Loading data...")
    ...
    model = XGBRegressor(n_estimators=100,learning_rate=L_R,subsample=subsample,colsample_bytree=col,max_depth=depth,
    ...
    #####
    results = [L_R,subsample,col,depth,lamb,apl,val_mmc_mean,corr_plus_mmc_sharpe,corr_plus_mmc_sharpe_diff,corr_with_example_preds]
    print(results)
...
def start():
    ...
    for n, m, l, k, j, i in itertools.product(range(len(apl)), range(len(lamb)), range(len(max_depth)),
                                              range(len(col_sample)), range(len(subsample)), range(len(learning_rate))):
        if __name__ == '__main__':
            results.append(main(learning_rate[i], subsample[j], col_sample[k], max_depth[l], lamb[m], apl[n]))
            print(n, m, l, k, j, i)
            ...
            for count in range(os.cpu_count()):
                ...
                processes.append(Process(target=start))
